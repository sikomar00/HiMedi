...
public class SceneNetworkManager : MonoBehaviourPunCallbacks
{
    ...
    //자신을 담을 static 변수
    public static SceneNetworkManager instance = null;

    //모든 Player들의 PhotonView 를 가지는 List
    public List<PhotonView> listPlayer = new List<PhotonView>();
    public GameObject[] playerCharacterUI;
    public GameObject[] playerCharacterIdxUI;
    ...
    void Start()
    {
        ...
        //나의 Player 생성
        Debug.Log("My player idx is " + idx);
        SetActiveUIList(idx);
    ...
    void SetInactiveUIList(int idx)
    {
        for (int i = 0; i <= idx; i++)
        {
            playerCharacterUI[i].SetActive(false);
            playerCharacterIdxUI[i].SetActive(false);
        }
    }

    void SetActiveUIList(int idx)
    {
        for (int i = 0; i <= idx; i++)
        {
            playerCharacterUI[i].SetActive(true);
            playerCharacterIdxUI[i].SetActive(true);
        }
    }
    ...
    public override void OnPlayerEnteredRoom(Photon.Realtime.Player newPlayer)
    {
        ...
        int currPlayerCnt = PhotonNetwork.CurrentRoom.PlayerCount - 1;
        print("Player" + currPlayerCnt + "님이 들어왔습니다!");

        SetInactiveUIList(currPlayerCnt);
        SetActiveUIList(currPlayerCnt);
    }
}
